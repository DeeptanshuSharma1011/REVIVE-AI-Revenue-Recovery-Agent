...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const normalizedActiveTab: NavTab = (() => {
    if (activeTab === 'simulator' || activeTab === 'simulate') return 'run_agent';
    if (activeTab === 'live_agent') return 'run_agent';
    if (activeTab === 'decisions') return 'run_agent';
  ...
  const primaryTabs: { id: NavTab; label: string; icon: React.ReactNode; badge?: string; badgeColor?: string }[] = [
    {
      id: 'overview',
      label: 'Overview',
          ...
          {primaryTabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-medium tracking-tight transition-all whitespace-nowrap cursor-pointer ${
                  isActive
